...
class AuthStatusDataSource {
  ...
  Future<Either<AppError, CustomUserModel>> authSatus({required String id}) async {
    try {
      /// check if current user is merchant or normal user
      ///
      final user = (await _firestore.collection(AppConstant.users).doc(id).get());
      if (user.exists && user.data() != null) {
        return right(CustomUserModel.fromJson(user.data()!));
      }
      final merchant = await _firestore.collection(AppConstant.merchants).doc(id).get();
