...
class BannerRemoteSource implements _BannerRemoteSource {
  ...
  Future<Either<AppError, List<BannerModel>>> getBanners() async {
    try {
      ...
      final response = await _reader.read(firestoreProvider).collection(AppConstant.banners).get();
      if (response.docs.isNotEmpty) {
        return right(response.docs.map((banner) => BannerModel.fromJson(banner.data())).toList());
      } else {
        return right([]);
      }
    } catch (e) {
      log(e.toString());
      return left(const AppError.serverError(message: 'Failed to Fetch Banners'));
    }
  }
}

final bannerRemoteSourceProvider = Provider<BannerRemoteSource>((ref) => BannerRemoteSource(ref));
