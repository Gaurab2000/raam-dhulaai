...
class UserSignupProvider extends StateNotifier<AppState<User?>> {
  ...
  Future<void> signup({
    ...
    final response = await _remoteSource.signupUser(name: name, email: email, password: password, location: location);
    state = response.fold(
      (error) => error.when(
...
final userSignupProvider = StateNotifierProvider.autoDispose<UserSignupProvider, AppState<User?>>(
